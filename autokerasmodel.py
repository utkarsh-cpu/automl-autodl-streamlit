...
def autokeras_classification(data,values,data_val,value_val,test_data,test_values,no_of_model_to_be_checked,epoch,loss_func,metrics):
    model=ak.StructuredDataClassifier(max_trials=no_of_model_to_be_checked,loss=loss_func,metrics=metrics,objective='val_accuracy')
    model.fit(data,encoded_data(values),validation_data=(data_val,value_val),epochs=epoch)
